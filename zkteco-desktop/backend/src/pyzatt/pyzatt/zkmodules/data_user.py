...
class DataUserMixin:
    def read_all_user_id(self):
        ...
        """
        self.send_command(
            cmd=DEFS.CMD_DATA_WRRQ, data=bytearray.fromhex("0109000500000000000000")
        )
        ...
        while i < total_size_dataset:
            ...
            if users_dataset[i + 3] != 0x00:
                password_bytes = users_dataset[i + 3 : i + 11].split(b"\x00")[0]
                password = password_bytes.decode("utf-8", errors="ignore").strip()
            else:
                password = ""

            # extract user name
            user_name_bytes = users_dataset[i + 11 : i + 35].split(b"\x00")[0]
            user_name = user_name_bytes.decode("utf-8", errors="ignore").strip()
            ...
            # extract the user id
            user_id_bytes = users_dataset[i + 48 : i + 57].split(b"\x00")[0]
            user_id = user_id_bytes.decode("utf-8", errors="ignore").strip()
    ...
    def get_verify_style(self, user_id):
        ...
        """
        self.send_command(
            cmd=DEFS.CMD_VERIFY_RRQ, data=struct.pack("<H", self.id_to_sn(user_id))
        )
        self.recv_reply()
    ...
    def read_all_fptmp(self):
        ...
        """
        self.send_command(
            cmd=DEFS.CMD_DATA_WRRQ, data=bytearray.fromhex("0107000200000000000000")
        )
        ...
        while i < total_size_dataset:
            ...
            tmp_size = struct.unpack("<H", fptemplates_dataset[i : i + 2])[0] - 6
            ...
            self.users[user_sn].set_user_fptmp(
                fp_index=fp_idx, fp_tmp=fp_tmp, fp_flag=fp_flg
            )
