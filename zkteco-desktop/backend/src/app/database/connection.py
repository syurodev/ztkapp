    ...
    def _migrate_devices_table(self, cursor):
        ...
        if 'device_type' not in columns:
            try:
                ...
                cursor.execute("ALTER TABLE devices ADD COLUMN device_type TEXT DEFAULT 'pull'")
                print("device_type column added successfully")

                # Update existing devices to have 'pull' type
                cursor.execute("UPDATE devices SET device_type = 'pull' WHERE device_type IS NULL")
    ...
    def _migrate_users_table(self, cursor):
        ...
        if 'employee_code' not in columns:
            try:
                ...
                print(f"Warning: Could not add employee_code column to users table: {e}")
    ...
    def _migrate_attendance_logs_table(self, cursor):
        ...
        if 'original_status' not in columns:
            try:
                ...
                cursor.execute('ALTER TABLE attendance_logs ADD COLUMN original_status INTEGER DEFAULT 0')
