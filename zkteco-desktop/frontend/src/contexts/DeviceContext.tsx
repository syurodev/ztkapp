...
export function DeviceProvider({ children }: DeviceProviderProps) {
  ...
  // Find primary device (is_primary = true)
  const primaryDevice =
    devices.find((device) => device.is_primary === true) || null;
  const primaryDeviceId = primaryDevice?.id || null;
  ...
  const startDeviceCapture = async (deviceId: string) => {
    ...
    try {
      ...
      console.error(`Error starting capture for device ${deviceId}:`, error);
      const errorMessage = getErrorMessage(error);
      toast.error(
  ...
  const stopDeviceCapture = async (deviceId: string) => {
    ...
    try {
      ...
      console.error(`Error stopping capture for device ${deviceId}:`, error);
      const errorMessage = getErrorMessage(error);
      toast.error(
  ...
  const getDeviceCaptureStatus = (
    ...
    return captureStatus?.devices.find((d) => d.device_id === deviceId) || null;
  };
