...
def add_device():
    ...
    try:
        ...
        if device_type == 'pull':
            ...
            current_app.logger.info(f"Testing connection to new pull device {data.get('name')} at {data.get('ip')}:{data.get('port', 4370)}")
            ...
            current_app.logger.info(f"Adding push device {data.get('name')} - skipping connection test")
            device_info = {
                'device_type': 'push',
...
def update_device(device_id):
    ...
    try:
        ...
        device_type = data.get('device_type', existing_device.get('device_type', 'pull'))
        ...
        if (data.get('ip') != existing_device.get('ip') or
            ...
            if device_type == 'pull':
                current_app.logger.info(f"Testing connection to updated pull device {device_id}")
                ...
                current_app.logger.info(f"Skipping connection test for push device {device_id}")
...
def sync_users_from_push_device(device_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                "error": "Failed to queue sync command"
            }), 500
...
def stop_all_device_capture():
    ...
    try:
        ...
        return jsonify({
            "message": "Đã tắt thu dữ liệu realtime cho nhiều thiết bị"
        })
    except Exception as e:
