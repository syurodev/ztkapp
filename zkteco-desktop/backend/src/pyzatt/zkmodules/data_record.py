...
class DataRecordMixin:
    def read_att_log(self):
        ...
        """
        self.send_command(
            cmd=DEFS.CMD_DATA_WRRQ, data=bytearray.fromhex("010d000000000000000000")
        )
        ...
        if not self.last_payload_data or len(self.last_payload_data) < 4:
            return

        # get number of log entries
        # The first 4 bytes are the size, the rest is the data
        # Each record is 40 bytes long
        att_count = struct.unpack("<I", self.last_payload_data[0:4])[0] // 40
        ...
        for idx in range(att_count):
            ...
            # user id
            user_id = (
                self.last_payload_data[i + 2 : i + 11]
    ...
    def read_op_log(self):
        ...
        """
        self.send_command(
            cmd=DEFS.CMD_DATA_WRRQ, data=bytearray.fromhex("0122000000000000000000")
        )
        self.recv_long_reply()
        ...
        op_count = struct.unpack("<I", self.last_payload_data[0:4])[0] // 16
    ...
    def clear_data(self, data_type=None):
        ...
        """
        if data_type:
            self.send_command(cmd=DEFS.CMD_CLEAR_DATA, data=bytearray(data_type))
        else:
