...
def sync_users_from_device(device_id=None):
    ...
    try:
        active_device = (
            config_manager.get_active_device()
            if not device_id
            else config_manager.get_device(device_id)
        )
        ...
        if device_type == "push":
            current_app.logger.info(
                f"Device {target_device_id} is push type, skipping user sync from device (push devices send data automatically)"
            )
        ...
        # Get users from device using the pyzatt-based service
        current_app.logger.info("Getting users from device via ZkService (pyzatt)...")
        zk_service = get_zk_service(device_id=target_device_id)
        device_users = zk_service.get_all_users()
        ...
        for device_user in device_users:
            try:
                ...
                if not device_serial:
                    ...
                    device_serial = (
                        device_info.get("serial_number") if device_info else None
                    )

                # Check if user already exists in database
                existing_user = user_repo.get_by_user_id(
                ...
                if existing_user:
                    ...
                    updates = {
                        ...
                        "privilege": getattr(
                            device_user, "privilege", existing_user.privilege
                        ),
                        "group_id": getattr(
                            device_user, "group_id", existing_user.group_id
                        ),
                        "card": getattr(device_user, "card", existing_user.card),
                        "password": getattr(
                            device_user, "password", existing_user.password
                        ),
                    ...
                    if has_changes:
                        ...
                        updated_count += 1
                        current_app.logger.info(
                            f"Updated user {device_user.user_id}: {device_user.name}"
                        )
                    else:
                        current_app.logger.debug(
                            f"User {device_user.user_id} already up-to-date, skipping..."
                        )
                    continue
                ...
                if source_user:
                    current_app.logger.info(
                        f"Found existing user with same ID on another device. Copying data from {source_user.device_id}."
                    )
                ...
                if created_user:
                    synced_count += 1
                    current_app.logger.info(
                        f"Created user {device_user.user_id}: {device_user.name}"
                    )

            except Exception as user_error:
                current_app.logger.error(
                    f"Error syncing user {device_user.user_id}: {user_error}"
                )
                continue

        current_app.logger.info(
            f"Sync completed: {synced_count} new users created, {updated_count} users updated"
        )
...
def create_user():
    ...
    try:
        ...
        current_app.logger.info(
            f"Creating user with ID: {user_id} and Data: {user_data}"
        )
...
def serialize_user(user):
    return {"id": user.user_id, "name": user.name, "groupId": user.group_id}
...
def sync_users():
    ...
    try:
        ...
        current_app.logger.info(
            f"Manual sync users requested for device: {device_id or 'active device'}"
        )
        ...
        current_app.logger.error(error_message)
        return jsonify(
            {"message": error_message, "success": False, "error": error_message}
        ), 500
...
def create_fingerprint(user_id):
    ...
    try:
        current_app.logger.info(
            f"Creating fingerprint for user with ID: {user_id} and finger index: {temp_id}"
        )
...
def delete_fingerprint(user_id, temp_id):
    ...
    try:
        current_app.logger.info(
            f"Deleting fingerprint for user with ID: {user_id} and finger index: {temp_id}"
        )
...
def get_fingerprint(user_id, temp_id):
    ...
    try:
        current_app.logger.info(
            f"Getting fingerprint for user with ID: {user_id} and finger index: {temp_id}"
        )
        ...
        return jsonify(
            {"message": "Lấy mẫu vân tay thành công", "data": serialized_template}
        )
    except Exception as e:
...
def sync_single_user(user_id):
    ...
    try:
        ...
        if not active_device:
            return jsonify(
                {"success": False, "message": "Không tìm thấy thiết bị đang hoạt động"}
            ), 400
        ...
        if sync_response.get("status") != 200:
            current_app.logger.error(
                f"API returned non-200 status: {sync_response.get('status')}, message: {sync_response.get('message')}"
            )
            return jsonify(
                {
                    "success": False,
        ...
        users_array = [
            {"id": payload_user_id, "serial_number": db_user.serial_number or ""}
        ]
        ...
        return jsonify(
            {
                ...
                "user_id": user_id,
                "employee_details": employee_details_data.get(user_lookup_key)
                if (employee_details_data and user_lookup_key)
                else None,
            }
        )
    ...
    try:
        ...
        if file and file.filename.endswith(".json"):
            ...
            if not exact_import:
                ...
                if not active_device:
                    return jsonify(
                        {"error": "No active device configured for import."}
                    ), 400
                ...
                try:
                    ...
                    if exact_import:
                        ...
                        if not target_device:
                            raise ValueError(
                                f"Device with serial_number '{serial_number}' not found"
                            )
                    ...
                    if existing_user:
                        updates = {
                            ...
                            "privilege": user_data.get(
                                "privilege", existing_user.privilege
                            ),
                            "group_id": user_data.get(
